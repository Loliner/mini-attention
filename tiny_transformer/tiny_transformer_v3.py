# 这是基于 v2 的优化版本，新增了：
# 1. 增加了多层 Block，提升模型的表达能力
# 2. 在 Block 内部加入了前馈网络，增强信息消化能力
# 3. 添加了 Dropout，防止过拟合
# 4. 在生成函数中加入了 top-k 采样，提升生成文本的质量
# 5. 引入 余弦退火（Cosine Annealing） 学习率调度器
import torch
...
class TinyTransformer(nn.Module):
    def __init__(self, vocab_size, d_model=16):
        ...
        self.blocks = nn.ModuleList([Block(d_model, n_head=4) for _ in range(layer)]) # 堆叠 layer 个 Block
...
generated_ids = model.generate(x_input, max_new_tokens=300)
