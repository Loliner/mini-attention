...
class SimpleAgent:
    ...
    def execute(self, plan):
        ...
        for step in plan:
            ...
            if tool == "calculate":
                for key, val in execute_cache.items():
                        args["expression"] = args["expression"].replace(f"step{key}", str(val))
                        # print(f"After replace: {args}")
            if tool in tools:
                result = tools[tool](**args)
                execute_cache[step_n] = result
        print(f"Execute cache: {execute_cache}")
        last_step = plan[-1]["step"]
        return execute_cache[last_step]
    
    def respond(self, user_input, result):
        respond_prompt = """
        ...
        response = client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": respond_prompt}, 
                      {"role": "user", "content": f"用户问题是：{user_input}，结果是：{result}，请直接给出一句话的简洁回答，不要推导过程"}]
        )
        content = response.choices[0].message.content
        return content
    ...
    async def async_run(self, user_input):
        ...
        response = self.respond(user_input, result)
